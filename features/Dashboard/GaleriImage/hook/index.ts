...
export const useGaleriFotoFeature = () => {
  ...
  const { data, isLoading, refetch } = useGetGaleriFoto({ value, page, limit });
  ...
  const { mutate: mutateEdit, isPending: isPendingEdit } = useMutation({
    mutationFn: async () => {
      const response = await axiosInstanceToken.put(
        `/v1/api/galeri-image/${selectedGaleri?.id}`,
        {
          image: images,
          title: form.getValues("title"),
        },
  ...
  useEffect(() => {
    if (selectedGaleri) {
      ...
      setImages(selectedGaleri.image);
    } else {
      form.reset({
        title: "",
      });
